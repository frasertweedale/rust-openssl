...
impl MlDsaKeyPair {
    ...
    pub fn from_seed(variant: Variant, seed: &[u8]) -> Result<Self, ErrorStack> {
        #[cfg(ossl350)]
        {
            ...
            unsafe {
                let evp = crate::cvt_p(crate::ffi::EVP_PKEY_new())?;
                let pkey = PKey::from_ptr(evp);
                crate::cvt(crate::ffi::EVP_PKEY_fromdata(
                    ctx.as_ptr(),
                    &mut pkey.as_ptr(),
                    crate::ffi::EVP_PKEY_KEYPAIR,
                    params.as_ptr(),
                ))?;
                Ok(MlDsaKeyPair { variant, pkey })
            }
    ...
    pub fn sign(&self, message: &[u8], context: Option<&[u8]>) -> Result<Vec<u8>, ErrorStack> {
        #[cfg(ossl350)]
        {
            use crate::pkey_ml_dsa::{sign_with_context, verify_with_context};
    ...
    /// Verify a signature on a message with optional context.
    ///
    /// Note: Verification is not yet implemented for BoringSSL backend.
    #[cfg(boringssl)]
    pub fn verify(
        &self,
        _message: &[u8],
        ...
        // TODO: Implement verification for BoringSSL
        // Would need to implement verify() on PKeyMlDsaParams<Public>
        Err(ErrorStack::get())
    }
...
mod tests {
    ...
    fn test_verify() {
        ...
        assert!(!keypair.verify(wrong_message, &signature, None).unwrap());
    }
}
